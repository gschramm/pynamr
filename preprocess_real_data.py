...
sdir1 = os.path.join(
    glob(os.path.join(pdir, '*TE05*'))[0],
    sdir.split('_')[0], sdir)
sdir2 = os.path.join(
    glob(os.path.join(pdir, '*TE5*'))[0],
    sdir.split('_')[0], sdir)
fpattern = '*.c?'
...
for i in range(ncoils):
    ...
    cimg2[i, ...] = np.flip(
        np.fromfile(fnames2[i],
                    dtype=np.complex64).reshape(data_shape).swapaxes(0, 2),
        (1, 2))
    cimg2_fft = np.fft.fftn(cimg2[i, ...], norm='ortho')
    data2[i, ...] = np.fft.fftshift(
        np.pad(np.fft.fftshift(cimg2_fft),
               (recon_shape[0] - data_shape[0]) // 2))
...
for sl in [45]:
    ...
    for i in range(8):
        ax.flatten()[i].imshow(np.abs(sens[i, :, :, sl]).T,
                               origin='lower',
                               cmap=plt.cm.Greys_r,
                               vmin=0,
                               vmax=0.4)
    ...
    ax.flatten()[8].imshow(np.abs(sos[:, :, sl]).T,
                           origin='lower',
                           cmap=plt.cm.Greys_r)
