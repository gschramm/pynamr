...
1. Convert the input proton MPRAGE to nifti.

2. configure input file names in a config.json file (start from demo_config.json)
     in the config.json you have to specify:
       ...
       - "MPRAGE_filename": abs. path of the MPRAGE nifti image

3. preprocess the sodium data, run N4 of the proton MPRAGE and align the proton MPRAGE
   ```
   python preprocess.py --cfg my_config.json
   ``` 
4. run 3 AGRs with different beta values (pre-defined in the recon parameter jsons)
   ```
