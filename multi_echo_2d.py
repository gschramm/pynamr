...
odir = os.path.join('data','recons_multi_2d', '__'.join([x[0] + '_' + str(x[1]) for x in args.__dict__.items()]))
...
#----------------------------------------------------------------------------------------------
# extract 2D data from 3D data
# do so, we calculate the fourier transform along the z axis and extract on slice
...
# we also have to regnerate the k-space values
# setup the frequency array as used in numpy fft
...
ifft          = np.zeros((ncoils,nechos,) + (n,n,2))
ifft_filtered = np.zeros((ncoils,nechos,) + (n,n,2))
abs_ifft          = np.zeros((ncoils,nechos,) + (n,n))
abs_ifft_filtered = np.zeros((ncoils,nechos,) + (n,n))
...
vmax = 1.5*abs_f.max()
ax1[0,0].imshow(Gam_recon.T, vmin = 0, vmax = 1, cmap = py.cm.Greys_r)
ax1[1,0].imshow((Gam_recon - Gam[...,slice2d]).T, vmin = -0.3, vmax = 0.3, cmap = py.cm.bwr)
ax1[2,0].imshow(abs_recon.T, vmin = 0, vmax = vmax, cmap = py.cm.Greys_r)
ax1[3,0].imshow((abs_recon - abs_f[...,slice2d]).T, vmax = 0.3, vmin = -0.3, cmap = py.cm.bwr)
...
for i in range(n_outer):
  ...
  ax1[2,i+1].imshow(abs_recon.T, vmin = 0, vmax = vmax, cmap = py.cm.Greys_r)
  ax1[3,i+1].imshow((abs_recon - abs_f[...,slice2d]).T, vmax = 0.3, vmin = -0.3, cmap = py.cm.bwr)
  ...
  ax1[0,i+1].imshow(Gam_recon.T, vmin = 0, vmax = 1, cmap = py.cm.Greys_r)
  ax1[1,i+1].imshow((Gam_recon - Gam[...,slice2d]).T, vmin = -0.3, vmax = 0.3, cmap = py.cm.bwr)
...
with h5py.File(output_file, 'w') as hf:
  ...
  grp.create_dataset('Gam',           data = Gam[...,slice2d])
  grp.create_dataset('Gam_recon',     data = Gam_recon)
  grp.create_dataset('ground_truth',  data = f[...,slice2d,:])
...
ax[0,0].imshow(abs_f[...,slice2d].T, vmax = vmax, cmap = py.cm.Greys_r)
ax[0,0].set_title('ground truth signal')
ax[1,0].imshow(Gam[...,slice2d].T, vmax = 1, vmin = 0, cmap = py.cm.Greys_r)
ax[1,0].set_title(r'$\Gamma$')
ax[0,1].imshow(abs_ifft_filtered[0,0,...].T, vmax = vmax, cmap = py.cm.Greys_r)
ax[0,1].set_title('filt. IFFT 1st echo')
ax[1,1].imshow(abs_ifft_filtered[0,-1,...].T, vmax = vmax, cmap = py.cm.Greys_r)
ax[1,1].set_title('filt. IFFT last echo')
ax[0,2].imshow(abs_ifft[0,0,...].T, vmax = vmax, cmap = py.cm.Greys_r)
ax[0,2].set_title('IFFT 1st echo')
ax[1,2].imshow(abs_ifft[0,-1,...].T, vmax = vmax, cmap = py.cm.Greys_r)
ax[1,2].set_title('IFFT last echo')
ax[0,3].imshow(abs_recon.T, vmax = vmax, cmap = py.cm.Greys_r)
ax[0,3].set_title(r'recon $\beta$ ' + f'{bet_recon}')
ax[1,3].imshow(Gam_recon.T, vmax = 1, vmin = 0, cmap = py.cm.Greys_r)
ax[1,3].set_title(r'$\Gamma$ recon $\beta$ ' + f'{bet_gam}')
ax[0,4].imshow((abs_recon - abs_f[...,slice2d]).T, vmax = 0.2, vmin = -0.2, cmap = py.cm.bwr)
ax[0,4].set_title(r'bias recon $\beta$ ' + f'{bet_recon}')
ax[1,4].imshow((Gam_recon - Gam[...,slice2d]).T, vmax = 0.2, vmin = -0.2, cmap = py.cm.bwr)
