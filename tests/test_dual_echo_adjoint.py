...
class TestModelAdjoint(unittest.TestCase):
  ...
  """
    ...
    def test_model_adjoint(self):
        ...
        sens = np.random.rand(ncoils, n_ds, n_ds,
                              n_ds).astype(np.float64) + 1j * np.random.rand(
                                  ncoils, n_ds, n_ds, n_ds).astype(np.float64)

        Gam = np.random.rand(n, n, n).astype(np.float64)

        readout_time = pynamr.TPIReadOutTime()
        kspace_part = pynamr.RadialKSpacePartitioner(data_shape,
                                                     n_readout_bins)

        m = pynamr.MonoExpDualTESodiumAcqModel(ds, sens, dt, readout_time,
                                               kspace_part)
